...
extern A_TYPE * char_type, * int_type, *float_type, *string_type, *void_type;
...
// 리터럴 테이블의 주소가 12번지를 가리키게 하고 루트 노드는 원래 위치에서 리터럴 테이블의 크기만큼 떨어지게 됨
void set_literal_address(A_NODE *node)
{
    int i;
    for (i = 1; i <= literal_no; i++)
        literal_table[i].addr += node->value; // 루트 노드의 전역주소 값? global_address를 마지막으로 넣긴한데 먼가 이상함
    node->value += literal_size; // 리터럴 테이블 크기만큼 떨어트림
}

// 프로그램의 각 변수의 크기, 주소를 계산, 점검
void sem_program(A_NODE *node)
{
    ...
    switch(node->name){
        case N_PROGRAM:
            ...
            node->value=global_address; // 마지막 전역 주소 값이 저장됨
            break;
        default :
...
int put_literal(A_LITERAL lit, int ll)
{
    if (literal_no >= LIT_MAX)
        ...
        literal_no++; // 리터럴 수 증가
    literal_table[literal_no] = lit; // 리터럴 addr 세팅
    literal_table[literal_no].addr = literal_size; // 현재 리터럴 주소? 크기. 상대주소인듯

    if(lit.type->kind == T_ENUM)
        literal_size += 4; // int인 경우 4씩 뜀
    else if (isStringType(lit.type)) // 문자열인 경우
        literal_size += strlen(lit.value.s) + 1; // 널문자 포함

    if (literal_size %4) // 4byte 정렬
        literal_size = literal_size/4*4+4;

    return (literal_no); // 정렬된 최종 증가 주소 리턴. 다음에 쓰임
}

// 해당 수식의 타입을 리턴, 해당 노드에 최종 타입과 lvalue인지 여부 저장됨
// 리터럴 테이블이 필요한 경우 저장도 해줌.
A_TYPE *sem_expression(A_NODE * node)
{
    ...
    A_LITERAL lit;
    int i;
    BOOLEAN lvalue=FALSE; // 기본적으로 false
    switch(node->name){
        case N_EXP_IDENT :
            id = node->clink;
            switch(id->kind){
                ...
                case ID_PARM :
                    result = id->type;
                    if(!isArrayType(result)) // 배열이 아니면 lvalue true
                        lvalue = TRUE;
                    break;
                case ID_FUNC :
                    result = id->type;
        ...
        case N_EXP_FLOAT_CONST :
            ...
            node->clink = put_literal(lit, node->line); // 테이블에 저장 테이블 주소를 가리킴
            result = float_type;
            break;
...
A_NODE * convertUsualUnaryConversion(A_NODE * node)
{
    ...
    if (t==char_type){
        ...
        t->size = 4;
        node = makeNode(N_EXP_AMP, NIL, node, NIL); // 함수를 주소취급해서
        node->type = t;
    }
    return node;
}
...
A_TYPE * convertUsualBinaryConversion(A_NODE * node)
{
    ...
    if(isFloatType(t1) && !isFloatType(t2)){
        ...
        semantic_warning(14, node->line);
        node->llink = makeNode(N_EXP_CAST, t2, NIL, node->rlink);
        node->llink->type = t2;
        result = t2;
    } else if (t1 == t2){
        result = t1;
