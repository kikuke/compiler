...
// a라는 오른쪽 노드 끝의 오른쪽에 NIL 노드 만들고 왼쪽 노드에 노드 b를 둠
...
// id 지점으로부터 이전 id들중 s와 같은 id가 있는지 찾음
...
// id 지점으로부터 현재 레벨에서 이전 id들중 s와 같은 id가 있는지 찾음
...
// ID의 종류가 비어있거나 struct 또는 enun의 필드가 비어있는 경우 에러
...
// 타입이나 스토리지 값이 비어있다면 각각 인티저, 오토로 설정
...
// merge & update specifier t와 s를 p에 합침
...
// check if identifier is already declared in primary expression 해당 id를 반환함
...
A_TYPE *getTypeOfStructOrEnumReIdentifier(T_KIND k, char *s, ID_KIND kk) {
    A_TYPE *t;
    A_ID *id;
    id = searchIdentifier(s, current_id);
    if (id)
...
// check function declarator and return type. current id가 파라미터의 맨 끝을 가리키게함
A_ID *setFunctionDeclaratorSpecifier(A_ID *id, A_SPECIFIER *p) {
    ...
    // check function identifier immediately befere '('
    if (id->type->kind != T_FUNC) {
        syntax_error(21, NULL);
        return (id);
    } else {
        id = setDeclaratorElementType(id, p->type); // 리턴타입 설정인듯
        id->kind = ID_FUNC;
    }
    // check redeclaration
    a = searchIdentifierAtCurrentLevel(id->name, id->prev);
...
// set declarator_list type and kind based on storage class ID의 중복검사와 타입을 설정해줌
A_ID *setDeclaratorListSpecifier(A_ID *id, A_SPECIFIER *p) {
    A_ID *a;
    setDefaultSpecifier(p);
    a=id;
    while (a) {
        if (strlen(a->name) && searchIdentifierAtCurrentLevel(a->name, a->prev)) // 중복선언
...
void checkVoidParamName(A_ID *id) {
    if (id->type == void_type && id->name != "") {
        syntax_error(14, id->name);
        return;
    }
}

// paramlist에서 void가 존재하는지 검사
void checkVoidParam(A_ID *id) {
    while (id) {
        if (id->level < current_level)
            break;
...
// set type initail value (expression tree)
...
void initialize() {
    ...
    setDeclaratorTypeAndKind(makeIdentifier("printf"),
        setTypeField(
            ...
            linkDeclaratorList(setDeclaratorTypeAndKind(makeDummyIdentifier(), string_type, ID_PARM), setDeclaratorKind(makeDummyIdentifier(), ID_PARM))),
        ID_FUNC);
